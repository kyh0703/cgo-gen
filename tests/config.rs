...
output:
  dir: gen
  basename: foo_wrapper
"#,
    )
    .unwrap();

    let config = Config::load(&config_path).unwrap();
    assert_eq!(config.output.basename.as_deref(), Some("foo_wrapper"));
    ...
    assert_eq!(config.output.go_filename("Foo"), "foo_wrapper.go");
}
