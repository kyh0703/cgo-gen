...
- facade method analysis와 Go rendering 분리
  - known model out-param이면 model-mapped path
  - 아니면 일반 facade API path
- unknown model reference/pointer declaration의 raw-first 유지
  - raw wrapper는 opaque handle 기반으로 계속 생성
  - Go facade/model projection에서는 `files.model` 기준으로만 승격 또는 노출
- raw-safe하지 않은 by-value object declaration의 explicit skip
  - header 전체를 abort하지 않고 declaration만 `support.skipped_declarations`에 기록
  - raw/Go output 모두에서 보수적으로 제외
...
현재 facade 확장은 **collection 의미 추론**보다 **명시적인 model-aware routing**을 이미 우선 적용한 상태다.

- known model type이 supported out-param 위치에 명시적으로 나타나는 API만 model-mapped facade 후보로 본다
- unknown model reference/pointer가 있어도 raw wrapper는 유지할 수 있으면 남긴다
- known model type이 없으면 Go facade/model projection에서는 보수적으로 제외한다
...
```text
pkg/sil/raw/is_aa_master_wrapper.h
pkg/sil/raw/is_aa_master_wrapper.cpp
pkg/sil/raw/is_aa_master_wrapper.ir.yaml
pkg/sil/model/is_aa_master_wrapper.go

pkg/sil/raw/is_aa_user_wrapper.h
pkg/sil/raw/is_aa_user_wrapper.cpp
pkg/sil/raw/is_aa_user_wrapper.ir.yaml
pkg/sil/facade/is_aa_user_wrapper.go
```
...
- Go facade에서 지원하지 않는 typedef primitive alias는 raw-only로 남을 수 있음
