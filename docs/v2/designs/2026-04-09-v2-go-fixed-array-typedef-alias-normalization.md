...
Fix generated Go wrappers so fixed arrays declared through primitive typedef aliases preserve the correct unsigned element type, eliminating the `sample_manager/public_wrapper.go` build errors around `ReasonCode[64]` and `SubscriptionId[...]`.
...
- Existing system facts:
  - `/workspace/vendor/public_api/MonitorDefs.h` declares `StatusInfo.PrimaryReasonCodes` and `StatusInfo.SecondaryReasonCodes` as `ReasonCode[64]`.
  - `/workspace/vendor/public_api/Public.h` declares `SubscriptionCodes.SubscriptionIds` as `SubscriptionId[...]`.
  - `ReasonCode` and `SubscriptionId` both resolve to `uint32`.
  - Generated C wrappers already preserve the unsigned ABI:
    - `sample_manager/public_wrapper.h` uses `unsigned int*` for `cgowrap__StatusInfo_SetPrimaryReasonCodes`, `cgowrap__StatusInfo_SetSecondaryReasonCodes`, and `cgowrap__SubscriptionCodes_SetSubscriptionIds`.
  - Generated Go facade is inconsistent:
    - `sample_manager/public_wrapper.go` currently emits `[]int32` and `*C.int32_t` for the same fields.
...
- User brief:
  - The representative generated `sample_manager` output fails with:
...
- Re-run generation or equivalent local verification for `sample_manager` and confirm the three failing sites no longer cast through `int32`.

### Success Criteria
- Generated Go setter/getter signatures for `ReasonCode[64]` and `SubscriptionId[...]` become `[]uint32`.
...
- Do not manually patch `sample_manager/public_wrapper.go`; the fix must come from generator output.
...
- `cargo test`
- Re-generate `sample_manager` locally and inspect the three previously failing call sites for `uint32`/`C.uint32_t`

### Parallelization Hints
- Candidate write boundaries:
  - `src/codegen/go_facade.rs`
  - `tests/`
  - generated `sample_manager/` output
...
- Likely sequential dependencies:
  ...
  - then re-generate `sample_manager` and validate the output
